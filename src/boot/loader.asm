...
InitKernel:
	xor esi, esi
    xor ecx, ecx
	mov cx, word [KERNEL_PHY_ADDR + 2Ch]	; cx = e_phnum (Program Header 数量)
	mov esi, [KERNEL_PHY_ADDR + 1Ch]		; esi = Program Header Table 在文件中的偏移
	add esi, KERNEL_PHY_ADDR				; esi <- OffsetOfKernel + pELFHdr -> e_phoff
.Begin:
	mov eax, [esi + 0]
	cmp eax, 0 				; PT_NULL
	je	.NoAction                       ; e_type = 0, 不可用的段
    ; e_type != 0, 这是一个可用段
	push dword [esi + 010h]				; MemCpy size 参数
	mov eax, [esi + 04h]				; eax = p_offset 段的第一个字节在文件中的偏移
	add eax, KERNEL_PHY_ADDR	        ; eax -> 段的第一个字节
	push eax							; MemCpy src 参数
	push dword [esi + 08h]				; MemCpy dest 参数
	call MemCpy							;
	add esp, 12							; 清理堆栈
.NoAction:
