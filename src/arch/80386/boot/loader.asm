...
NO_FILE:
    mov dh, 2
    call DispStr    ; "No kernel is found"
    ; 死循环
    jmp $
FILENAME_FOUND:
    ; 准备参数，开始读取文件数据扇区
...
Message:		    db	"Loader loading...."
                    db  "Loader too large!!"
                    db  "No kernel is found"
;============================================================================
; 16位支持函数库
;----------------------------------------------------------------------------
...
PM_32_START:            ; 跳转到这里，说明已经进入32位保护模式
    ...
    mov dword [BOOT_PARAM_ADDR], BOOT_PARAM_MAGIC   ; LettleOS引导参数的魔数
